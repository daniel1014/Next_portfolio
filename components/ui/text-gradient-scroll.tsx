...
function TextGradientScroll({
  ...
  const { scrollYProgress } = useScroll({
    target: ref,
    // Default offset reveals earlier (when the paragraph's top is ~80% down the viewport)
    // and completes before it reaches the very top, avoiding fixed-header issues.
    offset: (offset ?? ["start 80%", "end 20%"]) as any,
  });
